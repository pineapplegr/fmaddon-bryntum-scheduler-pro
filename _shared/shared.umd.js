...
window.bryntum.product = {
  ...
  appId: '8cfa4cf07fc1bf9694a01a3a627f5778d85f5c83',
  jumpWidth: '23em'
};
/* global RC */
// Load product config
...
const errorListener = errorEvent => earlyErrorEvent = errorEvent,
  ...
  shouldShow = (item, filterValue) => {
    var _item$cls;
    return !item.isMenuItem || ((_item$cls = item.cls) === null || _item$cls === undefined ? undefined : _item$cls['b-docs-category']) || item.text.toLowerCase().includes(filterValue);
  };
...
class Shared extends Localizable(Events()) {
              ...
              if (!this.items.length) {
                ...
                for (const meta of Object.values(bryntum.usedClasses)) {
                  ...
                  const {
                    ...
                    fullName = (_publicClasses$name = publicClasses[name]) === null || _publicClasses$name === undefined ? undefined : _publicClasses$name.replaceAll('.', '/');
  ...
  async initHints() {
    ...
    if (!hints || me.preventHints || ((_me$toolTips = me.toolTips) === null || _me$toolTips === undefined ? undefined : _me$toolTips.length) > 0) {
      return;
    }
    me.toolTips = [];
    ...
    if (!me.preventHints) {
      ...
      if (me.toolTips) {
        me.toolTips.forEach(tip => {
          var _tip$hide;
          return (_tip$hide = tip.hide) === null || _tip$hide === undefined ? undefined : _tip$hide.call(tip).then(() => tip.destroy());
        });
  ...
  startRootCause() {
    ...
    if ((_earlyErrorEvent = earlyErrorEvent) !== null && _earlyErrorEvent !== undefined && _earlyErrorEvent.error) {
  ...
  notifyIfLaterVersionExists(response) {
    ...
    const latestVersion = (_response$parsedJson = response.parsedJson) === null || _response$parsedJson === undefined ? undefined : _response$parsedJson.name;
  ...
  async showCodeEditor(noAnimation) {
    ...
    await codeEditor.expandPanel(noAnimation ? {
      animation: null
    } : undefined);
    codeEditor.focus();
    return codeEditor;
...
class VanillaCodeEditor extends DemoCodeEditor {
  ...
  applyChanges() {
    ...
    if ((_globalThis$logger = globalThis.logger) !== null && _globalThis$logger !== undefined && _globalThis$logger.active && !me.addedCodeChangeTag) {
  ...
  async updateJS() {
    ...
    try {
      ...
      imports === null || imports === undefined || imports.forEach(imp => {
      ...
      for (const project of projects) {
        ...
        (_project$destroy = project.destroy) === null || _project$destroy === undefined || _project$destroy.call(project);
      }
...
document.addEventListener('DOMContentLoaded', () => {
  ['grid', 'scheduler', 'schedulerPro', 'gantt', 'calendar', 'taskboard', 'histogram', 'treegrid'].forEach(productId => {
    ...
    if (!((_window$productId = window[productId]) !== null && _window$productId !== undefined && _window$productId.isWidget)) {
...
window.addEventListener('message', ({
  ...
  if (data) {
    ...
    if ((_message = message) !== null && _message !== undefined && _message.style) {
      const container = document.getElementById('container');
