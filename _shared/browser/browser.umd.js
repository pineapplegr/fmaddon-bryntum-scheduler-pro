...
class ExamplesApp {
    ...
    EventHelper.on({
      scroll: {
        handler() {
          ...
          jumpTo.value = (topElement === null || topElement === undefined || (_topElement$dataset = topElement.dataset) === null || _topElement$dataset === undefined ? undefined : _topElement$dataset.group) ?? null;
        },
        element: browserEl,
        throttled: 250
      },
      ...
      items: {
        ...
        filterField: {
          ...
          filterFunction(record, value) {
            ...
            return value === null || value === undefined ? undefined : value.toLowerCase().split(' ').every(word => ['title', 'version', 'folder'].some(param => {
              var _record$param;
              return (_record$param = record[param]) === null || _record$param === undefined ? undefined : _record$param.toLowerCase().includes(word);
            }));
          },
  ...
  onFocusIn({
    ...
    var _target$id;
    if (target !== null && target !== undefined && (_target$id = target.id) !== null && _target$id !== undefined && _target$id.startsWith('b-example')) {
      this.exampleElements.forEach(example => example.classList[example === target ? 'add' : 'remove']('b-focused'));
      window.location.hash = `#${target === null || target === undefined ? undefined : target.id.replace(/^b-/, '')}`;
    }
  }
            ...
            children: [{
              ...
              children: [example.title, {
                ...
                className: {
                  tooltip: 1,
                  'b-fa': 1,
                  'b-fa-cog build': hasPopup,
                  'b-fa-info-circle': !hasPopup
                }
              }]
            }, example.version && {
              className: 'version',
    ...
    Widget.attachTooltip(me.examplesContainerEl, {
      ...
      getHtml: async ({
        ...
        const linkNode = activeTarget.closest('a'),
          url = `${linkNode.getAttribute('href') || linkNode.dataset.linkUrl}/app.config.json`;
  ...
  onThumbError(e) {
    var _e$target;
    if ((_e$target = e.target) !== null && _e$target !== undefined && (_e$target = _e$target.src) !== null && _e$target !== undefined && _e$target.includes('thumb')) {
